...
CREATE TABLE Property (address VARCHAR(50) PRIMARY KEY, ownerName VARCHAR(30), price INTEGER);

CREATE TABLE BusinessProperty(type CHAR(20), size INTEGER, address VARCHAR(50), FOREIGN KEY(address) REFERENCES Property(address));

CREATE TABLE House ( bedrooms INTEGER, bathrooms INTEGER, size INTEGER, address VARCHAR(50), FOREIGN KEY(address) REFERENCES Property(address));

CREATE TABLE Firm (id INTEGER PRIMARY KEY, name VARCHAR(30), address VARCHAR(50));

CREATE TABLE Agent(agentId INTEGER PRIMARY KEY, name VARCHAR(30), phone CHAR(12), firmId INTEGER, dateStarted DATE, FOREIGN KEY(firmId) REFERENCES Firm(id) );

CREATE TABLE Buyer(id INTEGER PRIMARY KEY, name VARCHAR(30), phone CHAR(12), propertyType CHAR(20), bedrooms INTEGER, bathrooms INTEGER, businessPropertyType CHAR(20), minimumPreferredPrice INTEGER, maximumPreferredPrice INTEGER);

CREATE TABLE Listings(address VARCHAR(50), agentId INTEGER, mlsNumber INTEGER PRIMARY KEY, dateListed DATE, FOREIGN KEY(agentId) REFERENCES Agent(agentId) );
